...
const STATUS_CONFIG: Record<ReviewStatus, { label: string; color: string; bg: string; icon: string }> = {
  pending:  { label: 'Pending Review', color: AMBER, bg: '#fefce8', icon: 'time-outline' },
  approved: { label: 'Approved',       color: GREEN, bg: '#e6f9f2', icon: 'checkmark-circle-outline' },
  rejected: { label: 'Rejected',       color: RED,   bg: '#fff5f5', icon: 'close-circle-outline' },
};

// ── Stars display ──────────────────────────────────────────────────────────────
function Stars({ value, color = GOLD }: { value: number; color?: string }) {
...
// ── Star picker row ────────────────────────────────────────────────────────────
function StarPicker({
  label, value, onChange, color = GOLD,
}: {
        ...
        {[1, 2, 3, 4, 5].map(n => (
          <TouchableOpacity key={n} onPress={() => onChange(n)} hitSlop={{ top: 8, bottom: 8, left: 4, right: 4 }}>
            <Ionicons
              ...
              size={26}
              color={n <= value ? color : TEXT_MUTED}
            />
          </TouchableOpacity>
        ))}
      </View>
      ...
      <View style={s.badgeRow}>
        ...
        {review.is_anonymous && (
          <View style={s.anonBadge}>
            <Ionicons name="eye-off-outline" size={12} color={TEXT_MUTED} />
    ...
    return (
      <SafeAreaView style={s.flex}>
        ...
        <View style={s.centered}>
          <Ionicons name="star-outline" size={56} color={TEXT_MUTED} />
  ...
  return (
    <SafeAreaView style={s.flex}>
      ...
      {loading ? (
        ...
        <View style={s.centered}>
          <Ionicons name="filter-outline" size={48} color={TEXT_MUTED} />
...
const s = StyleSheet.create({
  ...
  filterScroll: { flexGrow: 0, backgroundColor: CREAM, borderBottomWidth: 1, borderBottomColor: HAIRLINE },
  ...
  filterPillText:   { fontSize: 13, fontWeight: '600', color: TEXT_MUTED },
  ...
  anonBadgeText: { fontSize: 11, fontWeight: '600', color: TEXT_MUTED },
  rejectedNote: {
  ...
  cardMeta: { flex: 1 },
  restaurantName: { fontSize: 15, fontWeight: '700', color: TEXT_DARK },
  cuisine: { fontSize: 12, color: TEXT_MUTED, marginTop: 1 },
  date: { fontSize: 11, color: TEXT_MUTED },
  ratingsRow: { flexDirection: 'row', gap: 20, marginBottom: 8 },
  ratingGroup: { gap: 3 },
  ratingLabel: { fontSize: 11, color: TEXT_MUTED, fontWeight: '500' },
  comment: { fontSize: 14, color: TEXT_MUTED, lineHeight: 20, marginTop: 4 },

  actions: {
    flexDirection: 'row', gap: 10, marginTop: 12,
    paddingTop: 12, borderTopWidth: 1, borderTopColor: CREAM,
  },
  editAction: {
...
const ep = StyleSheet.create({
  ...
  restaurantName: { fontSize: 14, color: TEXT_MUTED, marginBottom: 20 },

  ratingRow: {
    flexDirection: 'row', alignItems: 'center',
    justifyContent: 'space-between', marginBottom: 14,
  },
  ratingLabel: { fontSize: 14, fontWeight: '600', color: TEXT_DARK },
  ...
  commentLabel: { fontSize: 13, fontWeight: '600', color: TEXT_MUTED, marginBottom: 8 },
  commentInput: {
    borderWidth: 1.5, borderColor: HAIRLINE, borderRadius: 12,
    paddingHorizontal: 14, paddingVertical: 12,
    fontSize: 14, color: TEXT_DARK, minHeight: 90, backgroundColor: CREAM,
    textAlignVertical: 'top',
  },
  ...
  saveBtn: {
    marginTop: 16, marginBottom: 8, backgroundColor: DEEP_GREEN,
